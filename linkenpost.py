...
containers = [container for container in linkedin_soup.find_all("div", {"class": "feed-shared-update-v2"}) if
              'activity' in container.get('data-urn', '')]
...
def convert_abbreviated_to_number(s):
    # Ensure that 's' is a valid numeric-like string
    if not isinstance(s, str):
        return s  # Return as is if it's already a number
    ...
    if 'K' in s:
        ...
        try:
            # Try converting directly to an integer if it's purely numeric
            return int(s)
        except ValueError:
            # If conversion fails, return 0 or some other default value
            return 0
...
for container in containers:
    ...
    post_reactions = reactions_element[reactions_idx].text.strip() if reactions_element and reactions_element[
        reactions_idx].text.strip() != '' else 0
    post_reactions = convert_abbreviated_to_number(str(post_reactions))
    ...
    # Ensure post_comments is passed as a string to the function
    post_comments = convert_abbreviated_to_number(str(post_comments))
    ...
    # Ensure post_shares is passed as a string to the function
    post_shares = convert_abbreviated_to_number(str(post_shares))

    # Append data to posts_data list
    posts_data.append({
        "Post Text": post_text,
        "Media Link": media_link,
        "Media Type": media_type,
        "Post Date": post_date,
        "Likes Numeric": post_reactions,
        "Comments Numeric": post_comments,
        "Shares Numeric": post_shares
    })
